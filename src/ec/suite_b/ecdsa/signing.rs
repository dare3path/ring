...
impl Zeroize for EcdsaKeyPair {
    fn zeroize(&mut self) {
        ...
        trace_log!("!!!! after zeroized EcdsaKeyPair");
    }
}

impl Drop for EcdsaKeyPair {
    fn drop(&mut self) {
        trace_log!("!!! before dropping EcdsaKeyPair");
        self.zeroize();
        trace_log!("!!! after dropping EcdsaKeyPair");
    }
}
...
impl Drop for NonceRandomKey {
    fn drop(&mut self) {
        trace_log!("!!! before dropping NonceRandomKey");
        self.zeroize();
        trace_log!("!!! after dropping NonceRandomKey");
    }
}
